...
export default function PatientOverview() {
  ...
  useEffect(() => {
    apiGet('/patients/me/summary')
      .then(setSummary)
      .catch(() => setSummary(null)); // use hardcoded fallback
  }, []);

  // Use API data if available, otherwise fallback to hardcoded values
  const activeJourneys = summary?.activeJourneys ?? 2;
  const upcomingAppts = summary?.upcomingAppts ?? 2;
  const nextApptDate = summary?.nextApptDate ?? 'Dec 18';
  const referralsTotal = summary?.referralsTotal ?? 3;
    ...
    <div className="content fade-up">
      ...
      <div className="g32">
        ...
        <div className="col">
          ...
          <div className="card">
            ...
            <div className="cb">
              {[{day:'18',mo:'Dec',name:'Dr. Patel — Cardiology',detail:'10:30 AM · Mount Sinai',lang:'🌐 Igbo reminder set',chip:'cg',chipLabel:'Confirmed'},
                {day:'22',mo:'Dec',name:'Dr. Adeyemi — Follow-up',detail:'2:00 PM · Scarborough',lang:'🌐 Igbo · English',chip:'cv',chipLabel:'Scheduled'}].map((a,i)=>(
                <div key={i} style={{ display:'flex', alignItems:'center', gap:'.85rem', padding:'.85rem 0', borderBottom: i===0?'1px solid var(--border)':'none' }}>
                  ...
                  <span className={`chip chip-${a.chip.replace('c','')}`} style={{ flexShrink:0 }}>{a.chipLabel}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="g2">
        ...
        <div className="card">
          ...
          {[{ico:'❤️',title:'Cardiology — Dr. Patel',detail:'Routine · Referred Dec 1',chip:'cg',chipLabel:'Booked · Dec 18',steps:'3/4',width:'75%'},
            {ico:'🩸',title:'Hematology — LifeLabs',detail:'Blood work complete · Dec 10',chip:'cg',chipLabel:'Completed',steps:'4/4',width:'100%'},
            {ico:'🦴',title:'Orthopaedics — Sunnybrook',detail:'Semi-urgent · Pending booking',chip:'cy',chipLabel:'Pending',steps:'1/4',width:'20%',barColor:'linear-gradient(90deg,#d97706,var(--yellow))'}].map((r,i)=>(
            <div key={i} style={{ display:'flex', alignItems:'center', gap:'.7rem', padding:'.82rem 1.2rem', borderBottom: i<2?'1px solid var(--border)':'none' }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'.5rem', marginTop:'.32rem' }}>
                  <span className={`chip chip-${r.chip.replace('c','')}`}>{r.chipLabel}</span>
